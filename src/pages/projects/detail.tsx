...
const ProjectDetailPageContent: React.FC = () => {
  ...
  return <div className="space-y-6">
    <PageHeader eyebrow="Proyecto" title={project?.name ?? 'Cargando proyecto'} description={project?.description || 'Sin descripción.'} actions={<div className="flex gap-2"><PermissionAction permission="projects.update" onClick={openEdit}>Editar</PermissionAction><PermissionAction permission="tasks.create" variant="contained" onClick={() => navigate(`/tasks?create=1&projectId=${projectId}`)}>Crear tarea</PermissionAction></div>} />
    {error ? <Alert severity="error">{getApolloErrorMessage(error)}</Alert> : null}
    <div className="grid gap-6 xl:grid-cols-[1.2fr_0.8fr]">
    <SectionCard title="Tareas" badge={tasks.length}><div className="divide-y divide-border">{tasks.map((task) => <div className="flex flex-wrap items-center gap-3 py-3" key={task.id}><div className="min-w-0 flex-1"><p className="font-medium">{task.title}</p><p className="text-xs text-text-muted">{task.assigneeId ? userName(task.assigneeId) : 'Sin asignar'} · {formatDate(task.dueDate)}</p></div><PriorityChip priority={task.priority} /><TaskStatusChip status={task.status} /></div>)}{!tasks.length ? <p className="py-6 text-center text-sm text-text-muted">No hay tareas todavía.</p> : null}</div></SectionCard>
    ...
    <FormDialog open={editOpen} onClose={() => setEditOpen(false)} title="Editar proyecto" actions={<><Button onClick={() => setEditOpen(false)}>Cancelar</Button><Button variant="contained" disabled={!name.trim() || updateState.loading} onClick={() => void save()}>Guardar</Button></>}><TextField autoFocus label="Nombre" value={name} onChange={(event) => setName(event.target.value)} /><TextField label="Descripción" value={description} onChange={(event) => setDescription(event.target.value)} multiline minRows={3} /><TextField select label="Estado" value={project?.status ?? 'active'} disabled><MenuItem value={project?.status ?? 'active'}>{project?.status}</MenuItem></TextField></FormDialog>
  </div>;
};
